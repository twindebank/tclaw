...
| `router/` | Per-user agent lifecycle management. Owns goroutine lifetimes, directory setup, MCP server creation, tool registration. Builds per-channel tool overrides from `config.Channel` tool fields. Uses `channelpkg.Registry` for channel building. The only stateful orchestrator. |
...
| `channel/` | Transport abstraction. Core `Channel` interface, `FanIn()` multiplexer, `ChannelMap()` helper. `RuntimeStateStore` for per-channel runtime state (platform state, teardown state, pending done). `ChannelSecretKey()` for deriving secret store keys. `EphemeralProvisioner` interface for platform-specific channel lifecycle (provision, teardown, confirm, notify). `PlatformState` and `TeardownState` with extensible `json.RawMessage` data for platform-specific persistent data. `ActivityTracker` tracks per-channel busy state and provides `NotifyIdle()` for event-driven wake. |
| `channel/channelpkg/` | `ChannelPackage` interface and `Registry` for channel type registration. Mirrors `tool/toolpkg/` pattern — each channel type is a self-contained package. |
| `channel/socketchannel/` | Unix socket channel transport. Local-only (blocked in non-local environments). |
| `channel/stdiochannel/` | Stdin/stdout channel transport and oneshot mode. Local-only. |
| `channel/telegramchannel/` | Telegram Bot API channel transport, builder, provisioner, and platform state. |
| `channel/all/` | Channel registry aggregator — imports all channel packages and exposes `NewRegistry()`. |
...
- User `alice` → `GITHUB_TOKEN_THEO`, `FLY_TOKEN_THEO`
