...
The redirect URI must be set to your tclaw's OAuth callback URL (e.g. `https://tclaw.fly.dev/oauth/callback` for production, `http://localhost:9876/oauth/callback` for local). Monzo uses Strong Customer Authentication — after the browser flow, the user must approve access via the Monzo app.
