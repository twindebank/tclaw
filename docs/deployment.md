...
Hooks are registered in each user's `settings.json` at boot from `hooks.Manifest`, as HTTP hooks
pointing at a loopback listener tclaw starts for that user. Nothing needs installing, locally or in the
image: the hooks are part of the tclaw process.
