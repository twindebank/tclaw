...
- `channel_done` dev session cleanup — when `channel_done` is called on a channel that has active dev sessions, automatically cancel those sessions (same as `dev_cancel`). Dev channels are ephemeral by nature; the agent shouldn't need to manually clean up sessions before calling `channel_done`. Requires dev sessions to be tagged with the channel that created them.
