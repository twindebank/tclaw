...
- `channel_create` `initial_message` param — message delivered to the new channel as its first inbound message once it comes online, enabling ephemeral channels to self-start without a human kick-off message. Currently the agent must call `channel_send` after `channel_create` in the same turn (before restart), but admin has no outbound link to dynamically created channels so the kick-off can't always be sent.
