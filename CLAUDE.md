...
- `internal/hooks/` — Claude Code hooks, served over HTTP by tclaw on a per-user loopback listener. The only place a direct file write can be caught, and the only place the user's own words can be captured; an MCP tool never sees either.
- `internal/memorylayout/` — the paths and env vars the agent loop and the hooks must agree on.
...
The agent cannot write a rulebook. `internal/hooks` (HTTP hooks registered in each user's
`settings.json` and answered by tclaw itself) refuses a direct write to that directory, and
