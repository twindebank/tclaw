...
func TestLessonCapture(t *testing.T) {
	t.Run("classifies what the user sends", func(t *testing.T) {
		tests := []struct {
			...
			{"work nobody asked for", "I never asked for those to be merged", true},
			{"stopping before the work was done", "dont stop when you havent finished what i've asked", true},
			{"a brief that says not to stop early", "add a retry to the uploader, and don't stop until the tests are green", false},
