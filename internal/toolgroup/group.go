...
func AllGroups() []GroupInfo {
	return []GroupInfo{
		...
		{GroupChannelMessaging, "Send messages to other channels (channel_send), read conversation history (channel_transcript), check if channels are busy (channel_is_busy), and tear down the current channel (channel_done). Cannot create new channels. Also renders a markdown file into a PDF and sends it to the chat, which can carry a doc_-prefixed secret the agent cannot read."},
