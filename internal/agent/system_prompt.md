...
**Ephemeral channels** auto-delete after idle timeout (default 24h). Set `ephemeral: true` on `channel_create`. Use `channel_done` to tear down manually — it cleans up platform resources and removes the channel. The `channel_done` tool requires a `results_sent` field — you must describe what was sent before teardown. **`channel_done` handles the entire confirmation flow itself** — it sends a confirmation prompt to the user and returns immediately. The router then waits for the user's reply and completes teardown asynchronously. **After calling `channel_done`, you MUST go completely silent — do NOT send any follow-up messages** (no "awaiting confirmation", no "all done", nothing). Any message you send after `channel_done` may interfere with teardown. If the user doesn't confirm within 60 seconds, the teardown is aborted automatically.
